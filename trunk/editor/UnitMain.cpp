...
//    case 219: {APPLY(cell->bot -= 64) Key = 1; break;}
    case 'U': {APPLY(cell->bot += 64) Key = 1; break;}
    case 'J': {APPLY(cell->bot -= 64) Key = 1; break;}


//    case 'X': {APPLY(cell->mid += 64) Key = 1; break;}
//    case 'C': {APPLY(cell->mid -= 64) Key = 1; break;}

//    case 'P': {APPLY(cell->top += 64) Key = 1; break;}
//    case 'O': {APPLY(cell->top -= 64) Key = 1; break;}
    case 'I': {APPLY(cell->top += 64) Key = 1; break;}
    case 'K': {APPLY(cell->top -= 64) Key = 1; break;}
...
/*
     ...
     */
    case  45: {if(tc)tx=(tx-1)%num_tex;APPLY(cell->btx=tx);Key=1;break;}
    case  46: {if(tc)tx=(tx+1)%num_tex;APPLY(cell->btx=tx);Key=1;break;}
    case  36: {if(tc)tx=(tx-1)%num_tex;APPLY(cell->wtx=tx);Key=1;break;}
    case  35: {if(tc)tx=(tx+1)%num_tex;APPLY(cell->wtx=tx);Key=1;break;}
    case  33: {if(tc)tx=(tx-1)%num_tex;APPLY(cell->ttx=tx);Key=1;break;}
    case  34: {if(tc)tx=(tx+1)%num_tex;APPLY(cell->ttx=tx);Key=1;break;}
  ...
  if (ExportDialog->Execute())
  {
    ...
    __try
    {
      ...
      SL->Add(Format("\"%s\",\"%s\",\"%s\",{", ARRAYOFCONST((
        "Test Map",
        "Derek J. Evans and GAZIN Matthieu",
        "Welcome to Yeti3D - GP32 Port"))));
...
void TFormMain::ImportFromFile(String AFileName)
{
 String S;
 String T;
 ...
 S.sprintf("%s.pal",T);
  ...
  TFileStream* FS_pal = new TFileStream(S, fmOpenRead);
  try
  {
   FS_pal->ReadBuffer(&palette, FS_pal->Size);
  }
  __finally
  {
    FS_pal->Free();
...
// Calculated the LUA
  for(int a=0;a<64;a++)
  {
   for(int i=0;i<256;i++)
   {
    lua[a][i]=RGB555(palette[i][2]*a/16,palette[i][1]*a/16,palette[i][0]*a/16);  // oui B et R sont inversé,c'est moi qui est fait ça parceque sinon les couleurs sont pas correcte
//    lua[a][i]=RGB555(palette[i][0]*a/16,palette[i][1]*a/16,palette[i][2]*a/16);  // oui B et R sont inversé,c'est moi qui est fait ça parceque sinon les couleurs sont pas correcte
   }
  }
...
// load new one
  ...
  try
  {
   FS->ReadBuffer(&textures, FS->Size);
   num_tex =  FS->Size/(64*64);
  }
  __finally
  {
    FS->Free();
...
void __fastcall TFormMain::ImportTextureClick(TObject *Sender)
{
